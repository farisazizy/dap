...
3. Tidak bisa undo jika user sudah diperintahkan untuk menginputkan sesuatu (kecuali pada menu)
4. Ketika mengUpdate data mahasiswa, NIM harus diupdate juga. (tidak bisa hanya mengubah nama)
...
func main() {
	...
	for pilmenu != 6 { // Mengakses pilihan menu
		...
		if pilmenu == 1 { // Input Data
			...
			} else if pilmenu == 3 { // output nama mahasiswa dengan jumlah SKS tertentu.

				fmt.Print("Jumlah SKS: ")
				fmt.Scanln(&sks)      // menginputkan Jumlah SKS Mahasiswa yang dicari
				CariSKS(&ArrMhs, sks) // memanggil prosedur CarSKS (Binary Search)
...
func OutputMenu() {
	...
	fmt.Println("3. Output Nama Mahasiswa dengan jumlah SKS tertentu.")
...
//CariSKS adalah prosedur yang akan memeriksa jumlah sks
func CariSKS(ArrMhs *mhsw, sks int) {
	...
	for awal < akhir && ArrMhs[tengah].jumsks != sks {

		if sks > ArrMhs[tengah].jumsks {

			awal = tengah + 1
		} else if sks < ArrMhs[tengah].jumsks {
	...
	if ArrMhs[tengah].jumsks == sks {

		fmt.Println("Nama: ", ArrMhs[tengah].nama)
		fmt.Println("NIM: ", ArrMhs[tengah].nim)
		fmt.Printf("IP: %.2f\n", ArrMhs[tengah].ip)
		fmt.Println("Jumlah SKS: ", ArrMhs[tengah].jumsks)
		fmt.Println()
