...
func Menu() {
	...
	fmt.Println("9. Keluar dari Aplikasi.")
...
func DelMhs(nim string, ArrMhs *mhsw) {
	...
	if ArrMhs[i].nim == nim { // jika nim sudah cocok
		...
		ArrMhs[i].nim = ArrTemp[i] // Data dipindahkan ke array ArrTemp
...
func DelMK(nim string, ArrMhs *mhsw) {
	...
	if ArrMhs[i].nim == nim { // jika nim sudah cocok
		...
		if strings.ToLower(matkul) == strings.ToLower(ArrMhs[i].mk[j].nama) {
			...
			ArrMhs[i].mk[j].nama = ArrTemp[i] // Data dipindahkan ke array ArrTemp
...
func CreateMK(nim string, ArrMhs *mhsw) {
	...
	if ArrMhs[i].nim == nim {
		...
		if ArrMhs[i].mk[j].nama == "" {
			...
			k = 0
			for k < JumMK-1 && ArrMhs[i].mk[k].nama != matkul {
				k++

			}

			for ArrMhs[i].mk[k].nama == matkul {
