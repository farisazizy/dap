	...
	for pilmenu != 6 { // Mengakses pilihan menu
		...
		if pilmenu == 1 { // Input Data
			...
			if pilmenu == 1 {
				...
				fmt.Scanln(&min)
				fmt.Print("IP Maksimum: ")
				fmt.Scanln(&max)
				RentangIP(&ArrMhs, min, max)
...
func RentangIP(ArrMhs *mhsw, min float64, max float64) {
	...
	for i := 0; i < JumMhs; i++ {
		...
		if ArrMhs[i].ip >= min && ArrMhs[i].ip <= max {
