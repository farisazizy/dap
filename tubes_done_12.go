...
func main() {
	...
	for pilmenu < 1 || pilmenu > 6 { // Validasi input menu dari user harus pada interval 1-7
	...
	for pilmenu != 6 { // Mengakses pilihan menu
		...
		if pilmenu == 1 { // Input Data
			...
			if pilmenu == 1 {
				...
				fmt.Print("IP Minimum: ")
				fmt.Scanln(&min)

				RentangIP(&ArrMhs, min)
				// BIKIN VERSI BINARY WOI
			} else if pilmenu == 4 {
			...
			for pilmenu < 1 || pilmenu > 3 { // Validasi input menu dari user harus pada interval 1-7
			...
			for pilmenu < 1 || pilmenu > 5 { // Validasi input menu dari user harus pada interval 1-7
		...
		Menu() // Menampilkan Menu kembali setelah mengakses 1-10

		pilmenu = 0

		fmt.Scanln(&pilmenu)             // Memilih menu kembali setelah mengakses 1-10
...
func HapusMenu() {
	...
	fmt.Println("2. Hapus Nilai Mata Kuliah.")
	fmt.Println("3. Kembali.")
	fmt.Print("Menu Anda: ")
}
...
//RentangIP adalah prosedur yang akan memeriksa rentan IP sekaligus melakukan pencarian dengan Binary Search
